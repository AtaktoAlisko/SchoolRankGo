...
func (sc UNTTypeController) CreateUNTType(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Проверяем, что передан только один тип (First_Type или Second_Type)
		...
		// Проверяем существование First_Type, если он передан
		...
		// Проверяем существование Second_Type, если он передан
...
func (sc UNTTypeController) GetUNTTypes(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for rows.Next() {
			...
			// Заполняем данные только если они существуют
