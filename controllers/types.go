...
func (c *TypeController) GetFirstTypes(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		query := `SELECT 
						ft.first_type_id, 
						ft.first_subject_id, fs.subject AS first_subject_name, COALESCE(fs.score, 0) AS first_subject_score,
						ft.second_subject_id, ss.subject AS second_subject_name, COALESCE(ss.score, 0) AS second_subject_score,
						COALESCE(ft.history_of_kazakhstan, 0), 
						COALESCE(ft.mathematical_literacy, 0), 
						COALESCE(ft.reading_literacy, 0)
				  FROM First_Type ft
